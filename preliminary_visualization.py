...
def generate_geospatial_evolution(df, topic_name, start_year, end_year, step=1):
    ...
    if world is None:
        print(
            "Error: Could not load any world map. Please check your internet connection."
        )
        return
    ...
    for i, year in enumerate(years):
        ...
        if G.number_of_edges() > 0:
            ...
            nx.draw_networkx_edges(
                G, pos, ax=ax, width=weights, edge_color="#2c3e50", alpha=0.3
            )
    ...
    plt.savefig(
        f"figures/geospatial_evolution_{topic_name.replace(' ', '_')}_{start_year}_{end_year}.png",
        dpi=300,
    )
