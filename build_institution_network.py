...
def build_institution_geo_dict(papers_data):
    ...
    """
    geolocator = Nominatim(user_agent="research_evolution_project_v2")
    unique_insts = {}
    ...
    for paper in papers_data:
        ...
        # Focus ONLY on the first author (Lead Lab)
    ...
    print(f"Geocoding {len(unique_insts)} unique Lead Institutions...")
    for inst_id, data in unique_insts.items():
        try:
            # Better query: "University Name, Country"
            query = f"{data['name']}, {data['country']}"
            location = geolocator.geocode(query)
            if location:
                unique_insts[inst_id]['coords'] = (location.latitude, location.longitude)
            
            time.sleep(1.1) # Respect OpenStreetMap's 1-second rule
        except Exception as e:
            print(f"Error geocoding {data['name']}: {e}")
...
def build_edges_with_geo_dict(papers_data, geo_dict):
    ...
    for paper in papers_data:
        ...
        for inst in authorships[0].get('institutions', []):
            inst_id = inst.get('id')
            if inst_id in geo_dict and type(geo_dict[inst_id]['coords']) != float and geo_dict[inst_id]['coords'] is not None:
                if isinstance(geo_dict[inst_id]['coords'], str):
                    geo_dict[inst_id]['coords'] = tuple(map(float, geo_dict[inst_id]['coords'].strip('()').split(', ')))
...
def fetch_all_years_raw_data(topic_id, start_year=2010, end_year=2025, papers_per_year=100):
    """
